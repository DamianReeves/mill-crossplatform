...
def verify(ev:Evaluator) = T.command {
  locally {
    scalaVersions.foreach { scalaVersion =>
      val Result.Success(Watched(Some(Vector((taskName,options), rest @ _*)), _)) = execute(ev, s"""core._.jvm.__.scalacOptions""")
  ...
  locally {
    scalaVersions.foreach { scalaVersion =>
      val Result.Success(Watched(Some(Vector((taskName, options), rest@_*)), _)) = execute(ev, s"""core._.js.__.scalacOptions""")
      println(s"scalacOptions for $taskName: $options")
      assert(
        options == ujson.Arr("-deprecation", "-feature", "-unchecked", "-Xfatal-warnings"),
        "Scalac options are not correct")
  ...
  scalaVersions.foreach { scalaVersion =>
    core(scalaVersion).jvm.test.compile
    core(scalaVersion).js.test.compile
    core(scalaVersion).native.test.compile
  }
  ()
}