...
def verify(ev:Evaluator) = T.command {
  locally {
    scalaVersions.foreach { scalaVersion =>
      val Result.Success(Watched(Some(Vector((taskName,options), rest @ _*)), _)) = execute(ev, s"""nesting.core._.jvm.test.scalacOptions""")
  ...
  locally {
    scalaVersions.foreach { scalaVersion =>
      val Result.Success(Watched(Some(Vector((taskName, options), rest@_*)), _)) = execute(ev, s"""nesting.core._.js.test.scalacOptions""")
      println(s"scalacOptions for $taskName: $options")
      assert(
        options == ujson.Arr("-deprecation", "-feature", "-unchecked", "-Xfatal-warnings"),
        "Scalac options are not correct")
